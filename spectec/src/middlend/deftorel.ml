...
let rec collect_fcalls_exp iter_binds env e = 
  ...
  | CallE (id, args) when StringSet.mem id.it env.rel_set -> 
    ((fun_prefix ^ id.it $ id.at, args, e.note), iter_binds) ::
...
let create_fun_prem ids ((id, args, r_typ), iterexps) =
  ...
  let new_var, typ, prem = List.fold_left (fun (var, typ, prem) (iter, id_exp_pairs) -> 
    let new_typ = IterT (typ, iter) $ id.at in
...
let create_call_map fcalls binds = 
  let fcalls' = Util.Lib.List.nub (fun ((id, args, _), iterexps) ((id', args', _), iterexps') -> 
    Eq.eq_id id id' &&
...
and transform_exp call_map env e: exp * (id * typ) list = 
  ...
  | CallE (id, args) -> 
    ...
    | Some ({it = VarE id; note; _} as e') -> e'.it, [(id, note)] 
    | _ -> 
      let args', iter_ids_list = List.split (List.map (transform_arg call_map env) args) in
  ...
  | IterE (e1, (iter, id_exp_pairs)) -> 
    ...
    let new_id_exp_pairs = List.map (fun (id, typ) -> 
      let itert = IterT (typ, iter) $ typ.at in
      (id, VarE (apply_iter_to_var id.it iter $ id.at) $$ id.at % itert)
    ) iter_ids in
    let new_iter_ids = List.map (fun (id, typ) -> 
      let itert = IterT (typ, iter) $ typ.at in
      apply_iter_to_var id.it iter $ id.at, itert
    ) iter_ids in
...
and transform_arg call_map env a: arg * (id * typ) list =
...
let rec transform_prem call_map env prem = 
  (match prem.it with
  | RulePr (id, m, e) -> RulePr (id, m, transform_exp_normal call_map env e)
  | IfPr e -> IfPr (transform_exp_normal call_map env e)
  | LetPr (e1, e2, ids) -> LetPr (transform_exp_normal call_map env e1, transform_exp_normal call_map env e2, ids)
  | ElsePr -> ElsePr
  | IterPr (prem1, (iter, id_exp_pairs)) -> 
    
    IterPr (transform_prem call_map env prem1, 
      (transform_iter call_map env iter, List.map (fun (id, exp) -> (id, transform_exp_normal call_map env exp)) id_exp_pairs)
    )
  | NegPr p -> NegPr p
  ) $ prem.at
...
let transform_rule env rule = 
  ...
  | RuleD (id, binds, m, exp, prems) -> 
    ...
    List.map (transform_prem call_map env) (prems @ new_prems))
  ) $ rule.at
...
let transform_prod env prod = 
  ...
  | ProdD (binds, sym, exp, prems) -> 
    ...
    sym, 
    transform_exp_normal call_map env exp, 
    List.map (transform_prem call_map env) (prems @ new_prems)) $ prod.at
...
let must_be_relation env params clauses = 
...
let cvt_def_to_rel env id params r_typ clauses = 
  ...
  let rules = List.mapi (fun i clause -> 
    ...
    | DefD (binds, args, exp, prems) -> 
      ...
      RuleD (fun_prefix ^ id.it ^ "_case_" ^ Int.to_string i $ id.at, binds @ new_binds, new_mixop, tupe, List.map (transform_prem call_map env) (new_prems @ prems)) $ id.at
    ) clauses 
  in
  RelD (fun_prefix ^ id.it $ id.at, new_mixop, tupt, rules)

let rec transform_def (env : env) def = 
  let call_map = ExpMap.empty in
  (match def.it with
  | RelD (id, m, typ, rules) -> 
    RelD (id, m, transform_typ_normal call_map env typ, List.map (transform_rule env) rules)
  | DecD (id, params, typ, clauses) when must_be_relation env params clauses -> 
