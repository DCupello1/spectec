...
type pass_env = {
  mutable nat_subs_set: IdSet.t
}

let empty_pass_env = {
  nat_subs_set = IdSet.empty
}

let add_to_nat_set (env : pass_env) (id : id): unit =
  env.nat_subs_set <- IdSet.add id.it env.nat_subs_set
...
and transform_exp (env : pass_env) (exp : exp): exp =
  ...
  (match exp.it with
    ...
    | ProjE ({it = UncaseE (e, _); _} as e1, i) -> 
      let id = string_of_typ_name e.note in 
      if (IdSet.mem id env.nat_subs_set) 
        then CallE (string_of_typ_name e.note ^ map_func_suffix $ e.at, [ExpA e $ e.at]) 
...
let transform_type_creation (env : pass_env) (id : id) (inst : inst): inst * def' option =
  ...
  match deftyp.it with
    ...
    | VariantT typcases -> reconstruct_typ (VariantT (List.map (fun (m, (binds, t, prems), hints) ->
        (m, (List.map (transform_bind env) binds, transform_type env t, List.map (transform_prem env) prems), hints)
      ) typcases)) None
    | StructT typfields -> reconstruct_typ (StructT (List.map (fun (a, (binds, t, prems), hints) ->
        (a, (List.map (transform_bind env) binds, transform_type env t, List.map (transform_prem env) prems), hints)
      ) typfields)) None
    | AliasT typ -> reconstruct_typ (AliasT (transform_type env typ)) None

let transform_rule (env : pass_env) (rule : rule): rule =
  (match rule.it with
    | RuleD (id, binds, m, exp, prems) -> RuleD (id, List.map (transform_bind env) binds, m, transform_exp env exp, List.map (transform_prem env) prems)
  ) $ rule.at

let transform_clause (env : pass_env) (clause : clause): clause =
  (match clause.it with
    | DefD (binds, args, exp, prems) -> DefD (List.map (transform_bind env) binds, List.map (transform_arg env) args, transform_exp env exp, List.map (transform_prem env) prems)
  ) $ clause.at
...
let rec transform_def (env : pass_env) (def : def): def list = 
  (match def.it with
    | TypD (id, params, insts) -> let (new_insts, opts) = List.split (List.map (transform_type_creation env id) insts) in
      [TypD (id, List.map (transform_param env) params, new_insts)] @ List.filter_map (fun d -> d) opts
