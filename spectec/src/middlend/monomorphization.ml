...
let print_env (m_env : monoenv) = 
  print_endline "Printing the Dependent Types Env: ";
  print_endline " ";
...
(* Simple AST traversal that collects all dependent types *)
...
let split_used_types_in_params (params : param list) (return_type : typ): param list * param list = 
  partition_map_using_tail (fun p ps -> 
    ...
    map_bool_to_either p (Free.Set.mem id_p (Free.Set.union free_vars_params free_vars_rt))
  ) params
...
let split_used_types_in_params_index (params : param list) (return_type : typ option): int list = 
  filter_map_using_taili (fun p ps i ->
    ...
    in
    let free_vars_params = (Free.free_list Free.free_param (List.filter check_dep_typ_in_params ps)).varid in
    map_bool_to_option i (Free.Set.mem id_p (Free.Set.union free_vars_params return_typ_ids))
  ) params
...
let _split_used_dependent_types_in_case_exp (exps : exp list) (case_typ : typ) : exp list * exp list =
...
(*
 * The following "get_all_case_instances" functions attempts to recursively go into a type and
 * grab all of the potential cases that it might be. 
 *
 * For example, say we have the types:
 * syntax numtype = I32 | I64 | F32 | F64
 * syntax vectype = | V128
 * syntax valtype = numtype | vectype
 * Then, say we want all cases for valtype, then it will recursively go into the type and grab
 * [I32, I64, F32, F64, V128]. 
 *
 * A special case is when we consider tuple types. Then we have to do the cartesian product between
 * all of the fields.
 * 
 * These two functions throws an exception "UnboundedArg" when we encounter a type that is simply not 
 * possible to get all of its cases (for example, nat or list)
 *)
...
let rec transform_exp (m_env : monoenv) (subst : subst) (exp : exp): exp =
  ...
  (match exp.it with
    | VarE _ -> (Il.Subst.subst_exp subst exp).it
    ...
    | CaseE (m, e) -> 
      ...
      if c_args_used = [] 
        ...
        else CaseE (transform_mixop m_env m (List.length c_unused_args) (List.map t_func c_args_used), TupE (c_unused_args) $$ e.at % e.note)
    ...
    | CallE (id, args) when not (Il.Env.mem_def m_env.il_env id) -> 
      (* Function parameter, leave it for now *)
    ...
    | CallE (id, args) ->
      ...
      let (used_args, unused_args) = partition_mapi (fun a i -> map_bool_to_either a (List.mem i used)) subst_args in
      if used_args <> [] && check_reducible_args used_args 
        then
          (let new_id = transform_id_from_args id (List.map (reduce_arg m_env.il_env) used_args) in
          function_calls_bind m_env id.it (CallE (new_id, List.map (reduce_arg m_env.il_env) used_args) $$ exp.at % exp.note);
          CallE (new_id, unused_args))
        else
          CallE (id, List.map (transform_arg m_env subst) args)
...
and transform_type (m_env : monoenv) (subst: subst) (typ : typ): typ = 
  (match typ.it with
    | VarT (id, []) when not (Il.Env.mem_typ m_env.il_env id) -> 
      (* Type parameter, don't do anything to it *)
      VarT (id, [])
    | VarT (id, args) when args <> [] ->
      let subst_args = List.filter (fun a -> not (is_type_arg a)) (List.map (transform_arg m_env subst) args) in 
      if subst_args <> [] && check_reducible_args subst_args 
        then (
          let reduced_args = List.map (fun arg -> reduce_arg m_env.il_env arg) subst_args in 
...
(* TODO deal with potential of type params *)
...
let transform_type_creation (m_env : monoenv) (id : id) (params : param list) (inst : inst) : def' list =
  match inst.it with 
    | InstD (binds, args, deftyp) -> 
      let non_type_args = List.filter (fun a -> not (is_type_arg a)) args in
      let transform_deftyp func = match args, StringMap.find_opt id.it m_env.concrete_dependent_types with
        ...
        | _, None when (List.for_all is_type_arg args) -> 
          (* If it only has type parameters, leave it for now *)
        ...
        | _, Some set_ref ->
          List.filter_map ( fun t -> 
            let dep_args = get_dependent_type_args t in 
            let subst = Option.value (try Il.Eval.match_list match_arg m_env.il_env Il.Subst.empty dep_args non_type_args with Irred -> None) ~default:Il.Subst.empty in
      ...
      | VariantT typcases -> 
        transform_deftyp (fun subst -> VariantT (List.concat_map (transform_type_case m_env subst) typcases) $ deftyp.at)
    )
      ...
      if not (check_matching m_env used_call_args used_args) then []
        ...
        if dep_binds == [] 
          then ([DefD (List.map (transform_bind m_env new_subst) (bind_exists binds new_subst), 
                  ...
                  transform_exp m_env new_subst exp, 
            ...
            List.map (fun clause_subst -> 
              ...
              List.map (fun arg -> transform_arg m_env subst_union arg) unused_args, 
              transform_exp m_env subst_union exp, 
    ...
    | RuleD (rule_id, binds, mixop, exp, prems) ->
      ...
      then [RuleD(rule_id, List.map (transform_bind m_env Subst.empty) binds, mixop, transform_exp m_env Subst.empty exp, List.map (transform_prem m_env Subst.empty) prems) $ rule.at] 
        ...
        List.filter_map (fun subst ->
          ...
          Some (RuleD (new_id, List.map (transform_bind m_env subst) unused, mixop, transform_exp m_env subst exp, new_prems) $ rule.at)
        ) subst_list)

let rec transform_def (m_env : monoenv) (def : def) : def list =
  (match def.it with
    | TypD (id, params, [inst]) when Mono_nat_subset.check_normal_type_creation inst -> transform_type_creation m_env id params inst
...
let transform (script: Il.Ast.script) =
  ...
  print_endline "Monomorphizing Dependent types...";
  ...
  print_env m_env;
  Mono_type_params.transform transformed_script